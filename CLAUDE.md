...
## v3.0 Phase 2 + B2 + B3 — LLM NLQ · persistence · scorecard · remediation
...
- **B4** multi-file + schema drift (reuse `dataset_signature`) ← suggested next, and the first step toward the owner's connectors vision.
- Then **B5** analytics NLQ (time-series/comparisons via the LLM plan), **B6** backend (Docker Postgres + scheduler + alerting + root-cause = the moat).
