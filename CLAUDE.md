...
## v3.0 Phase 2 + B2 + B3 + B4 + Autopilot — LLM NLQ · persistence · scorecard · remediation · multi-source · autopilot
...
- **`autopilot.py`** — the **Autopilot decision engine** (autonomous orchestrator on top of the existing engines; adds NO new scoring/cleaning logic). Feed it a manifest + scored frame + a `DEFAULT_POLICY` (per-dimension minimums + `max_critical_pct`/`max_anomaly_pct` ceilings) and `evaluate()` returns one explainable verdict — **APPROVE / REMEDIATE / QUARANTINE** — plus a confidence (decisiveness = avg distance of each gate from its threshold), the `policy_checks()` gates that drove it, a prioritised `action_plan` (reuses `remediation.suggested_actions`), and `next_steps`. `decide()` rule: QUARANTINE if a hard gate breaches (`max_critical_pct` exceeded, or overall/accuracy/consistency fall ≥`HARD_MARGIN`=15 below target); APPROVE if all gates pass; else REMEDIATE. `run_autopilot()` = build manifest + decide in one call. Report builders: `report_markdown` / `report_json` / `checks_df` / `action_plan_df`. Pure, deterministic, stdlib+pandas — designed to be reused by the B6 scheduler/alerting layer.

**app.py wiring:** sidebar "🤖 Smart Query" expander (opt-in, default OFF; provider/model/key/cost-cap/privacy toggle) → builds `LLM_CFG` + session `CostGuard`; `_smart_nlq()` router replaces BOTH `parse_nlq` UI call sites (Ask tab + Dashboard quick bar) and shows an engine badge; new **"🎯 Scorecard" tab (2nd tab, `tab_sc`)** = SLA target inputs → PASS/AT-RISK/FAIL status + per-area pass/fail grid + printable HTML export + optional prev-manifest deltas; new **"🛠️ Remediation" tab (`tab_rem`, after Export)** = remediation log + suggested actions + cleaned-CSV / log-MD / log-JSON downloads; new **"🕒 History" tab (`tab10`)** = download/upload manifests + trend lines + deltas. new **"🛸 Autopilot" tab (3rd tab, `tab_ap`, after Scorecard)** = quality-policy expander → verdict hero card (APPROVE/REMEDIATE/QUARANTINE + confidence) + "why" + policy-gate table + prioritised action plan + next steps + Markdown/JSON decision report & cleaned-CSV downloads (CSV disabled while QUARANTINE). NOTE on tab vars: extra tabs were inserted by NAME at the matching list position (`tab_sc` at pos 2, `tab_ap` at pos 3, `tab_rem` between `tab8`/`tab9`), so the original `tab1`–`tab10` still map to their tabs unchanged — no renumbering (`tab_ms` Multi-Source inserted between `tab_rem`/`tab9`). **14 tabs total.** `.streamlit/secrets.toml` is git-ignored — put `LLM_API_KEY` there.

Verified: `py_compile` clean; `_selftest_phase2.py` = **53/53** offline checks (Phase 2 + B2 + B3 + B4); `_selftest_autopilot.py` = **28/28** offline checks (Autopilot verdicts/gates/confidence/reports); headless Streamlit **AppTest** drives demo→score→**14 tabs** with **no exceptions** (Autopilot tab present at pos 3). Both `_selftest_*.py` are local-only regression tests (not committed; promote to `tests/` later).
...
- ✅ **Autopilot DONE** — `autopilot.py` + **🛸 Autopilot tab**: policy → explainable APPROVE/REMEDIATE/QUARANTINE verdict + confidence + gate table + action plan + decision report. Branch `claude/agitated-dhawan-283720`. *Follow-up:* feed the verdict into the B6 scheduler/alerting (auto-quarantine + notify on FAIL); per-COLUMN policy once per-column scores land in the manifest.
- **B5** analytics NLQ (time-series/comparisons via the LLM plan) ← suggested next.
- Then **B6** backend (Docker Postgres + scheduler + alerting + root-cause = the moat) — Autopilot's `evaluate()` is the decision rule it will call.
