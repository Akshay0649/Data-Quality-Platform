...
class SettingsDialog(tk.Toplevel):
    def __init__(self, parent, config, on_save):
        ...
        tk.Button(self, text="Save", bg=ACCENT, fg="#000",
                  ...
                  command=self._save).grid(row=len(fields), column=0,
                                          columnspan=2, pady=14)

    def _save(self):
        for key, var in self._vars.items():
            raw = var.get()
            cur = getattr(self._config, key)
            if isinstance(cur, float):
                try:
                    raw = float(raw)
                except ValueError:
                    pass
            setattr(self._config, key, raw)
        self._config.save()
        self._on_save()
        self.destroy()
...
class ParakeetOverlay:
    def __init__(self, config, on_ask, on_screen_capture, on_audio_toggle,
                 on_provider_change, on_resume_load, on_settings_save):
        self.config = config
        self.on_ask = on_ask
    ...
    def start_answer(self, question):
        self._token_q.put((Q_SEP, "\n" + "-" * 44 + "\n"))
        self._token_q.put((Q_QUESTION, "Q: " + question + "\n\n"))

    def stream_token(self, token):
        self._token_q.put((Q_ANSWER, token))

    def show_error(self, message):
        self._token_q.put((Q_ERROR, "  " + message + "\n"))

    def end_answer(self):
        self._token_q.put((Q_END, "\n"))
    ...
    def _apply_screen_share_invisibility(self, root):
    ...
    def _build_ui(self, root):
        ...
        header.pack(fill="x")
        tk.Label(header, text="Parakeet Personal",
                 bg=BG2, fg=ACCENT, font=("Helvetica", 13, "bold")).pack(side="left", padx=12)
        self._provider_var = tk.StringVar(value=self.config.ai_provider)
        for p in ("claude", "openai"):
            tk.Radiobutton(header, text=p.capitalize(), variable=self._provider_var,
                           value=p, bg=BG2, fg=FG, selectcolor=BG2,
                           activebackground=BG2,
                           command=self._on_provider_change).pack(side="right", padx=4)
        tk.Label(header, text="Model:", bg=BG2, fg=MUTED,
                 font=("Helvetica", 9)).pack(side="right", padx=(8, 0))

        self._status_var = tk.StringVar(value="Ready")
        tk.Label(root, textvariable=self._status_var, bg=BG2, fg=MUTED,
        ...
        self._response.tag_configure("sep", foreground="#2a2f45")
        self._response.tag_configure("question",
            foreground="#6b7280", font=("Helvetica", 10, "italic"))
        self._response.tag_configure("answer", foreground="#e2e4ea", font=mono)
        ...
        tk.Label(inp_frame, text="Question  (Enter=ask, Shift+Enter=newline)",
                 bg=BG, fg=MUTED, font=("Helvetica", 9)).pack(anchor="w")
        self._input = tk.Text(inp_frame, bg=BG2, fg=FG, insertbackground=FG,
                              height=3, font=("Helvetica", 11), wrap="word",
                              relief="flat", bd=6)
        self._input.pack(fill="x")
        self._input.bind("<Return>", self._on_enter)
        self._input.bind("<Shift-Return>", lambda _: None)
        ...
        def btn(parent, text, cmd, fg=FG, bg=BTN_BG, bold=False):
            ...
            b = tk.Button(parent, text=text, command=cmd, bg=bg, fg=fg,
                          activebackground=BTN_HOVER, font=f, relief="flat",
    ...
    def _on_listen_toggle(self):
        ...
        if self._listening:
            self._listen_btn.config(text="Stop", bg="#7f1d1d", fg=FG)
        else:
    ...
    def _poll_tokens(self):
        try:
            while True:
                ...
                tag, text = item
                if tag is Q_END:
                    self._response.insert("end", text)
                    self.set_status("Done")
                else:
                    self._response.insert("end", text, tag)
        except queue.Empty:
            pass
